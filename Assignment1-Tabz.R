...
#importing the data.
fin_data = read.csv("C:/Users/HP/Downloads/Train_v2.csv")
...
relationship_with_head2 = fin_dataK %>%
  mutate(relationship_with_head2 = ifelse(relationship_with_head %in% grep("relative",relationship_with_head, ignore.case = TRUE, value = T), "other", ""))
...
empower = fin_dataK %>%
  filter(gender_of_respondent %in% grep("Female", gender_of_respondent,ignore.case = TRUE, value = T)) %>% 
  filter(marital_status %in% grep("Single/Never Married", marital_status,ignore.case = TRUE, value = T)) %>% 
  filter(cellphone_access %in% grep("No", cellphone_access,ignore.case = TRUE, value = T)) %>% 
  filter(bank_account %in% grep("Yes", bank_account, ignore.case = TRUE, value = T))
